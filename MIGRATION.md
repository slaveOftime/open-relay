...
oly 1.0 is a **clean break** from 0.x. It will not read your old session
recordings, and old binaries will not read the new ones. Your data is never
deleted by upgrading — but you must back it up yourself before switching.

Read this file fully before upgrading a machine that has sessions you care
about.

## Why 1.0 breaks compatibility

0.x accumulated three parallel ways of doing the same thing: two persisted
...
silently disagree — and did. 1.0 keeps exactly one implementation of each
concern, chosen by the ADRs in `docs/adrs/`:

- **Storage:** the per-session shadow journal is the only persisted stream
...
Keeping 0.x readers alive would mean shipping the old bugs as permanent
compatibility branches. We chose a single supported stack plus an explicit,
honest migration instead.

## What breaks

| Area | 0.x | 1.0 | What you must do |
|---|---|---|---|
| Session recordings | `output.log` + `events.log` per session | `journal/` directory per session (segmented, incarnation-fenced) | Nothing readable automatically; see "Preserving old recordings" below. 1.0 prints an explicit error for pre-1.0 sessions instead of guessing. |
| CLI ↔ daemon protocol | IPC protocol v4 (and later nightly bumps) | IPC protocol **v12** | CLI and daemon must both be 1.0. A mismatch exits with a precise version error — no silent misbehavior. |
| Attach stream framing | base64 chunks inside JSON lines | binary frames after the JSON init line | Only relevant to tooling that spoke IPC directly; update it or pipe through the 1.0 CLI. |
| Federation | 0.x node handshake | fenced, deadline/keepalive-driven node links (M5-2…M5-4) | All nodes must run 1.0; re-`join` secondaries after upgrading. No rolling mixed-version federation. |
...
Finish or stop every session you still need interactively. A running 0.x
session belongs to the 0.x daemon; 1.0 cannot adopt it.
...
`events.log`. Keep it somewhere the 1.0 daemon does not write.
...
Do not start the 1.0 daemon while a 0.x daemon is running on the same state
directory.

### 4. Install 1.0 and start

```sh
cargo install --path .        # or your package manager of choice
oly daemon start
oly daemon status             # should report the 1.0 daemon
```
...
```sh
oly list                    # 0.x sessions appear; logs/attach report the
                            # pre-1.0 format explicitly, never garbage
oly start -- echo hello     # create a fresh 1.0 session
...
There is **no automatic import**, on purpose. A faithful 1.0 journal needs
...
- `output.log` contains the raw PTY byte stream. Replay it with any
  terminal-emulator tooling, e.g. `cat output.log` in a wide terminal, or
  `tmux new-session 'cat output.log; read'`.
- `events.log` lines are newline-delimited JSON — greppable for lifecycle
  and resize history.
- Old sessions remain listed in 1.0 with size 0; asking for their logs or
  attaching produces an explicit `uses the pre-1.0 log format (output.log) …
  see MIGRATION.md` error rather than empty output.

## Rollback

1. `oly daemon stop` (the 1.0 daemon).
2. Move the 1.0 state aside: `mv ~/.local/state/oly ~/.local/state/oly-1.0`.
3. Restore: `cp -a ~/.local/state/oly-0.x-backup ~/.local/state/oly`.
4. Reinstall the 0.x binary and start it.

A 0.x binary will never be pointed at 1.0 state, and 1.0 journals are not
promised to be readable by any 0.x build.

## Notes for tool authors

- IPC: requests and one-shot responses are newline-delimited JSON envelopes
  (`{"version":12,"payload":…}`). Attach streams send one JSON init line,
