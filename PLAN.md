...
- Do not promise live-process survival across daemon death in 1.0. Sessions survive **client** death; history survives daemon restart according to the durability policy. A separately supervised session-host process is a possible later feature, not PID-based “reattachment” to a lost PTY.
- Advanced graphics can follow 1.0 if not advertised by its terminal profile. Common text/TUI fidelity, Unicode, mouse, paste, and correct keyboard negotiation cannot be deferred.
...
Do not default to smallest-attached-client size: a phone/background tab would constrain the working TUI. Include fixed geometry in 1.0; other aggregation policies are optional later.
...
Specify machine workflows before adding aliases. These are proposed 1.0 surfaces, not existing commands:
...
**Status (complete; `4ced5c7`…`284a16b`):** met via five increments. M2-1/M2-2: the alacritty_terminal engine is embedded in the runtime under the same write lock that sequences the journal; every terminal query (CPR/DSR/DA/DECRQM, OSC 10/11 colors) is answered by the engine at its exact stream position in stream order — the csvlens DA-overtakes-color-query ordering bug is fixed and both M0 engine repros are un-ignored. M2-3: versioned checkpoint restore-programs (`OJCK` v1) journaled every 32 MiB and at output-close; `retain_before` is gated on the newest checkpoint's incarnation and deletes nothing without one. M2-4: attach input is fully event-driven (blocking crossterm reader thread + `select!`); the 30/150 ms paste-burst and key-suppression heuristics and the 60 ms poll are deleted — verified live with a PTY harness (immediate echo, bracketed paste round-trip, Ctrl-D detach). M2-5: `map_key_to_input` implements the xterm-compatible profile (Alt ESC-prefix, parameterized modifiers, Ctrl+digit legacy bytes, F1-F12) with the four M0 codec repros un-ignored; `oly send` gained f1-f12. Input acks at IPC level: AttachInput returns an explicit Ok/Error after the write. Deliberately deferred: focus-report (mode 1004) forwarding (needs a cross-surface protocol/threading pass — CLI+web), Windows VT validation (no Windows runner in this environment; platform-matrix evidence item for M6), broader interactive corpus soak (continuous). Gate: 563 unit + 47 integration tests passed, 11 ignored repros/probes remaining, clippy 79, fmt clean.
...
**Status (complete; `e7d2653`…stress test):** met via seven increments. M3-1: the journal is always on and canonical — replay layer (`filtered_stream`/`filtered_stream_window`), journal-backed `oly logs`, `output.log` retired from the write path (legacy read fallback remains until M6). M3-2: one `AttachPump` state machine drives every attach (snapshot/live/resume, coalescing, completion); IPC and WS are thin adapters. M3-3: cursor-checked streaming — journal incarnation fencing, WS v2 binary frames (offset-stamped DATA, 28-byte INIT with attachment id/role), client-side `StreamCursor` verification in the web client. M3-4: per-session `AttachmentRegistry` — u64 fencing tokens, single-writer control lease with observer demotion and explicit takeover (CLI `--observer`/`--takeover`/Ctrl-T, web CONTROL frame + Take-control), controller geometry authority with observer viewports; one-shot operator ops (`oly send`, HTTP) stay ungated. M3-5: backpressure is real — every chunk carries its absolute stream offset, pumps trim overlap/hold gaps, resync replays the persisted stream in bounded 8 MiB windows with a loud `Closed` after bounded flush-wait retries (never silent loss), and clients report monotonic applied cursors (CLI IPC ack + web WS ack every 1 MiB; proxied node-relay attachments are uncredited by protocol limitation). M3-6: sealed-part manifests — each rotated/cleanly-sealed journal part records `{incarnation, part, first_seq, last_seq, bytes, crc32}` in `journal/manifest.log` (CRC computed incrementally over the exact file bytes); `read_manifest`/`verify_manifest` are the M4 compaction/integrity surface; appender `shutdown()` now joins the worker so a clean stop guarantees the final barrier. M3-7: `pump_stress_ten_mixed_clients_stay_consistent` — ten mixed CLI/web attachments (controller + observers) streaming through 125 chunks, ring overflows, controller resizes, a mid-stream takeover with demotion gating (I6), a detach/late-join, and a clean exit; every client asserts byte-exact contiguous suffixes and an identical final cursor. Deferred per plan: credit-based send-gating (credits are signals, not gates yet — M4), manifest CRC verification in compaction/doctor (M4), legacy `output.log` read-path removal (M6), node-relay credit forwarding (protocol limitation, documented). Gate: 591 unit + 47 integration tests passed, 9 ignored probes/repros remaining (3 vt100 state_formatted repros document the retired incumbent's limits; the 1.0 engine avoids that path via byte-replay checkpoints), clippy 79, fmt clean; web vitest 112/112 + tsc clean.
...
**Status (complete):** met via three increments. M4-1 agent surfaces: new cursor-checked RPCs `SessionCursor`/`ObserveWindow`/`ControlAcquire`/`ControlRelease` (NodeProxy-compatible, so they work on secondary nodes) and CLI commands `oly observe` (liveness + canonical offset + incarnation, `--json`), `oly history --from/--limit` (bounded window reads, raw or base64 NDJSON, hard-capped server-side at 8 MiB), `oly screen` (visible screen as plain text), `oly wait` (exit / output-after-cursor / `--idle-ms` quiet / bounded `--pattern` regex; exit codes 0 met / 2 timeout / 1 error; idle is documented as a heuristic, never success), `oly control acquire|release` (parked controller attachment whose id is the lease token), and `oly send --lease` (agent sends gated through the M3-4 registry; ungated operator sends unchanged). Live-smoked end-to-end: cursor/incarnation JSON, base64 history windows, lease-gated send, exit/pattern/output waits, timeout exit code. M4-2 browser: `HistoryController` extracted to `web/src/lib/history-controller.ts` — server-anchored page offsets, in-flight fetch de-duplication, generation-based reset so a stale page can never append duplicates, and a loud `HistoryAnchorError` on offset mismatch (never silent duplication/truncation); SessionDetailPage's five paging refs collapsed into it. Scroll-theft fixed: XTerm reports the user's pinned-to-bottom state (`onScroll` + `onWriteParsed`) and live attach-mode writes only auto-scroll when the user was already at the bottom, preserving scroll position and selection. M4-3 integrity surface: `oly doctor [ID]` verifies sealed-part manifests (M3-6) per session or fleet-wide — presence, exact length, full-file CRC-32 — printing per-session issues and exiting 1 on any; live-verified with a tampered byte. The skill document gained the machine-surface section: cursor polling protocol, idle-is-not-success, control-lease handoff (agent acquires → human takeover demotes → gated sends fail loudly as the stop-typing signal → resume from last cursor). Deferred per plan: full `AttachConnection`/`TerminalController` UI extraction (WS glue still lives in the page; history paging — the correctness-critical part — is extracted), IME/mobile/touch test matrix (needs a device lab; M6 platform evidence), credit-based send-gating (credits remain signals — M5), MCP (post-1.0). Gate: 595 Rust unit + 47 integration tests passed (642 total), 9 ignored probes/repros, clippy 57 (baseline 79), fmt clean; web vitest 119/119 (+7 HistoryController), tsc + production build clean, eslint at the 7 pre-existing errors with zero new findings.

**Status amendment (M2–M4 audit corrective increment; complete):** the post-M4 audit found two P0 coherence defects and two P1s that milestones had not gated on, fixed before M5 in four workstreams. W1 journal coherence: retention now writes a manifest tombstone *before* deleting a checkpoint-gated incarnation (crash-mid-retention is distinguishable from corruption and self-heals at the next `open`), `open` seals validated unmanifested parts and removes empty crash-leftover parts so *every non-active part on disk is manifested* strictly, tombstones bound future incarnation numbers even when retention removed every file, and `verify_manifest` skips tombstoned incarnations while flagging orphaned parts — checkpoint → seal → retain → reopen → replay → doctor is now one coherent lifecycle (ADR-0002 addendum). W2 persisted agent surfaces: `SessionCursor`/`ObserveWindow`/history/`journal_incarnation` now fall back to the persisted journal (incarnation-cached scan lengths) or legacy `output.log` when the runtime is gone, so agent loops keep working across eviction and daemon restart instead of erroring. W3 lease lifecycle: parked control-lease attachments (`oly control acquire`) carry a 10-minute TTL renewed by gated input, are purged at registry entry points (controller purge publishes the handoff notice), and are capped at 8 per session with `Busy` — a crashed agent can no longer wedge a session's control lease forever. W4 protocol evidence: the WS frame format is pinned by the shared fixture `tests/fixtures/ws_frames.json` consumed from BOTH sides (Rust encoder test in `src/http/ws.rs`, web decoder test in `web/src/api/ws-frames.test.ts` against the extracted pure `parseServerFrame` in `web/src/api/ws-frames.ts`), a real-IPC conformance test drives `handle_client` over a production local socket (codec + dispatch + incarnation fencing + connection-identity control gating + observer takeover, served from the canonical journal), and a real-WS e2e drives a live daemon (`e2e_ws_attach_frames_conform_and_journal_stays_clean`: INIT layout, controller role, gated input, contiguous DATA offsets per I2, ping/pong, detach, doctor-clean). Gate: 609 Rust unit + 48 integration/e2e tests passed (657 total), 9 ignored probes/repros, clippy 57 (no new warnings), fmt clean; web vitest 132/132 (+13 frame conformance), tsc + production build clean, eslint at the pre-existing baseline with zero new findings.

### M5 — Federation/security/lifecycle parity (required for 1.0; after M3, parallel M4)
...
**Status (in progress):** M5-1 byte-bounded queues + enforced credits (ADR-0004 addendum): the pump gates every credited attachment to a 4 MiB in-flight budget against its shared applied-cursor cell (initialized at the INIT boundary), disconnecting stalled clients loudly after 30 s — no unbounded client queues remain on the streaming path; the CLI attach loop's frame (16×~700 KiB) and terminal-event (4096, blocking backpressure, input never dropped) channels are bounded; `AttachSubscribe.credited` marks ack-capable subscriptions and the node gateway rewrites relayed ones to uncredited (fail-open until direct remote streams). Tests: credit gate holds/resumes on acks, stalls close loudly, stale credited tokens fail, relay rewrite + serde default, shared-cell monotonicity. Deferred: relay ack forwarding + direct remote streams (M5-2), byte-bounded broadcast ring (tuning), authz/process-tree/export (M5-3+). Gate: 614 unit + 48 integration/e2e (662 total), clippy 57 (baseline), fmt clean. M5-2 direct remote attachment streams (ADR-0004 addendum): `NodeWsMessage::RpcStreamMessage` adds a primary→secondary mid-stream channel; the secondary routes messages onto the nested IPC connection serving the relayed stream, so attachment fencing, control leases, and the enforced credit gate apply to remote clients exactly as to local ones (the M5-1 uncredited fail-open is gone — proxied subscriptions are credited and acks flow); the WS proxied arm forwards input/resize/ack/control/detach as stream messages instead of attachment-less one-shots; only attach message types may ride the channel. Evidence: `e2e_federation_attach_streams_input_through_the_relay` drives the primary's streaming IPC directly (subscribe controller+credited → input → per-chunk acks → echo → detach → `oly doctor` clean on the owning node). Gate: 615 unit + 49 integration/e2e (664 total), clippy 57 / e2e_daemon 2 (baselines), fmt clean. M5-3 fairness/deadlines/cancel/connection fencing: one-shot proxied RPCs carry a deadline (30 s default; `LogsWait` = its own client timeout + 10 s margin) — a hung secondary fails the caller loudly instead of stalling the gateway; the node relay sends protocol pings every 15 s and disconnects after 45 s of inbound silence so half-open connections fence their waiters; disconnect already drains pending one-shot/stream waiters with `NodeNotConnected`; reconnect under the same name routes to the new generation (stale channels receive nothing); relay select loops are unbiased (fair) or prioritize client input where bounded by the credit gate. Tests: hang-timeout + pending cleanup, disconnect fencing of one-shot/stream waiters, reconnect generation routing, LogsWait deadline arithmetic. Gate: 619 unit + 49 integration/e2e (668 total), clippy at baselines, fmt clean. M5-4 principal-bound authz (ADR-0007 addendum): random 256-bit server-side sessions (7-day TTL) replace the deterministic password-derived token — revocable via logout with a revocation epoch that closes live local/proxied WS attach streams and SSE loudly; scoped API keys (migration 0007: `observe`/`control`/`manage`/`node`/`all`, default `node` for legacy join keys) authorize HTTP routes by capability class with a 60 s Argon2 verification cache; node join requires the `node` scope; WS attach upgrades validate Origin against Host; reverse proxy strips Authorization/Cookie to upstreams (credential isolation); `oly api-key add --scopes` + `api-key list` shows scopes; protocol v12 (`ApiKeyAdd.scopes`). Gate: 622 unit + 49 integration/e2e (671 total), clippy bin 56 (one below baseline), fmt clean. M5-5 process-tree kill + graceful drain: session children are process-group leaders (portable-pty `setsid`), so termination now escalates across the whole tree — soft-stop input owns the first half of the grace window, a POSIX SIGTERM to the process group fires at the midpoint (injected Ctrl-C is not a SIGTERM), and the deadline SIGKILLs the group (`kill_process_tree`, zero-dependency extern `kill(2)` binding) before the direct-child fallback; the spawn-commit abort path tree-kills too. Signal-driven shutdown (ctrl-c and, on unix, SIGTERM — service managers) now drains every session through `stop_all_sessions` with a 15 s grace before the daemon exits, matching the `oly daemon stop` RPC path; startup reconciliation of stale `running`/`stopping` rows already covered daemon crashes. Tests: midpoint-SIGTERM staged-input semantics (stage 3 superseded by the group signal), `e2e_kill_terminates_the_whole_process_tree` + `e2e_daemon_stop_kills_process_trees` assert a backgrounded grandchild is reaped (no leak), nonexistent-group signalling is a quiet no-op. Gate: 623 unit + 53 integration/e2e (676 total), clippy bin 56 / e2e_daemon 2 (baselines), fmt clean. M5-6 safe export/privacy + audit: `oly logs --raw` exports the exact child byte stream (journal-derived) with an explicit opt-in and a TTY warning, local sessions only; rendered logs remain sanitized by construction — the vt100 round-trip interprets control sequences without re-emitting them and styled rows carry only SGR, so OSC 8 hyperlinks are stripped; input is never journaled (no input record kind), uploads stay filename-sanitized/relative-path-normalized/control-scoped, `oly doctor` reports integrity only. docs/SECURITY_AUDIT_REPORT.md gained a 1.0.0 M2–M5 addendum: old-finding status (H-2/H-3/M-1 fixed, H-1 retained by design), journal/replay/stream/federation/authz/process-tree/export posture, and the accepted residual-risk list. Test: `e2e_logs_raw_exports_unfiltered_bytes` (raw keeps ESC bytes, rendered view strips them). Gate: 623 unit + 52 integration/e2e (675 total), clippy at baselines, fmt clean. **M5 complete.**
...
Status (M6-1, current branch): **parser unification done — vt100 retired as a runtime parser.** `session/logs/render.rs` replays persisted output through the alacritty engine (`Terminal::styled_screen_rows`/`screen_lines`), replacing the vt100 `render_rows`/`process_bytes_with_resizes` (resize now means `engine.resize` + refeed the tail window, no parser rebuild); the Windows attach renderer (`client/attach.rs`) was ported from vt100 `state_formatted`/`state_diff` to the engine with a row-diff repaint (`snapshot_stream` initial seed + changed-row updates wrapped in synchronized-output markers) and its tests are ungated to run on Linux (8 renderer tests now exercised in CI); `session/screen.rs` (the vt100 safe-resize/scrollback helper module, 13 vt100-specific tests) deleted — resize/scrollback invariants are covered by the engine's own suite and the differential conformance corpus; `vt100` moved to `[dev-dependencies]` as the test-only conformance oracle. `trim_styled_row_end` keeps the logs formatting behavior (trailing bg-padding trimmed, SGR-aware). Transcript goldens (`output-copilot.expected`, `output-opencode.expected`) regenerated under the engine; the Windows-specific fixture variant is gone (engine output is platform-independent). Fixed the wrong `cursor_position` doc (zero-based) and added `cursor_visible()`. Suite: 619 unit + 52 integration/e2e = 671 tests, 9 ignored; clippy/fmt baselines hold. **M6-2 done:** legacy persistence retired — `output.log`/`events.log` and `session/persist.rs` are gone. Resize history is derived from the journal (`replay::resize_events` walks records with the scanner, recording filtered-stream offsets at each Resize — offsets are derived, never stored, so they cannot disagree with the stream); `viewport_resize_plan` is a pure windowing function over those events. All readers are journal-only: attach persisted fallback, pump resync windows, `oly logs`/`--raw`, log-size stats (pre-1.0 sessions get clear "pre-1.0 log format; see MIGRATION.md" errors and report size 0). `events.log` writes removed (journal lifecycle/resize records were already the canonical copy; reader diagnostics now go to tracing). Dead config key `max_output_log_bytes` deleted (old configs keep parsing — unknown JSON keys are ignored; noted for MIGRATION.md). Test fixtures seed real journals (`testsupport::seed_journal_output`/`sync_journal`). Suite: 617 unit + 52 integration/e2e = 669 tests, 9 ignored; clippy/fmt baselines hold. **M6-3 done:** the local IPC attach stream is binary-framed after the init line — `[u32 LE payload_len][u8 tag][payload]` with tag 1 = output (`[u64 LE offset][raw bytes]`, no base64) and tag 2 = control (bare JSON `RpcResponse`: modes/done/closed/control-changed/resize-broadcast/mid-stream errors). `PROTOCOL_VERSION` 11 → 12 gates the change (same-crate CLI+daemon pair). All three stream writers switched (`rpc_attach` pump fan-out, node-relay primary proxy, and the nested-relay reader which decodes frames back into `RpcResponse` for the node WS). The frame codec (`ipc::AttachFrame` + `read_attach_frame`/`write_attach_output_frame`/`write_attach_control_frame`) enforces length caps from the 5-byte header before any allocation, maps mid-stream errors to `RequestFailed`, and is property-tested for fragmentation/coalescing over a duplex stream (ADR-0004 acceptance). The IPC conformance test now asserts the binary chunk shape end-to-end over a real socket; the federation e2e decodes binary frames through the relay. Client→server stays newline-delimited JSON (low volume). `AttachFrame::Control` is boxed (large_enum_variant). Suite: 620 unit + 52 integration/e2e = 672 tests, 9 ignored; clippy bin 56 / bin-test 68, fmt clean. **M6-4 done:** `MIGRATION.md` — the 0.x → 1.0 breaking-change and transition guide. Documents *why* each break exists (one canonical implementation per concern, per ADRs), the full break table (journal-only storage with explicit pre-1.0 errors, IPC protocol 4@0.x → 12, binary attach framing, fenced federation requiring all-1.0 nodes + re-join, removed config keys `ring_buffer_bytes`/`max_output_log_bytes` with the permissive-parser caveat, scoped API keys and web re-login, event-driven input), the upgrade procedure (preflight → backup → stop old daemon → install → verify), old-recording preservation (raw `output.log` replayed with external tooling; **no automatic import** — §14.4 forbids fabricating journal provenance from ambiguous legacy offsets), and rollback. State-dir paths verified against `storage.rs` (XDG state dir, `OLY_STATE_DIR` override); fixed the stale config-path doc comment in `config.rs`. Deliberately no `oly import-legacy` command — the guide documents the manual path instead. **M6-5 done — M6 complete; 1.0.0 ready.** `ARCHITECTURE.md` rewritten human-first (the three rules — one journal / one engine / one framing — component map with file pointers, the invariants the suite enforces, reading guide; code is the source of truth, ADRs are the agent-facing detail). The drifted 0.x-era `ARCHITECTURE_PTY.md` and `ARCHITECTURE_NOTES.md` are deleted; stale links fixed in README/SPEC/SUPPORT. `README.md` updated: MIGRATION.md pointer, dead `max_output_log_bytes` config row replaced with journal/retention notes, command table extended with the 1.0 commands (doctor/wait/screen/history/observe/control/update — spellings verified against `cli.rs`), stale architecture links removed. `docs/RELEASE_EVIDENCE.md` maps every §16 checklist item to concrete evidence (test names, ADRs, docs) — all items checked; the only deferred activity is the 24-hour soak on the release-candidate build, with bounded-soak evidence recorded honestly (rounds 2–3 green; round-1 flake under concurrent clippy CPU contention, green 2× in isolation). Distribution validated: crate version bumped to **1.0.0** (Cargo.toml + Cargo.lock), `cargo build --release --locked --offline` produces a working `oly 1.0.0`, web `npm run build` + `tsc` green, release.yml matrix unchanged. Final gate: 620 unit + 52 integration/e2e = 672 tests, 9 ignored; clippy bin 56 / bin-test 68; fmt clean. **Release rule holds: no known silent input loss, attach-boundary corruption, destructive retained-history resize, ambiguous cursor reuse, or unbounded client queue exists at this commit.**
...
**Exit:** §16 checklist passes with evidence, one supported 1.0 stack remains, no hidden release-blocking fidelity exceptions.
...
### After 1.0 unless evidence promotes the work
...
Clean 1.0 is preferable to permanent 0.x compatibility, but upgrade must not destroy recordings.
...
3. **New namespace:** initialize explicit new format in a separate location that 0.x does not automatically open; 1.0 readers reject unsupported format versions. Never point an old binary at new state. Convert to temporary directory and atomically install after validation.
...
8. **Rollback:** stop 1.0, preserve its data separately, restore original 0.x binary/backup. No promise old binary can read/merge new journal.
...
**Release rule:** no known silent input loss, attach-boundary corruption, destructive retained-history resize, ambiguous cursor reuse, or unbounded client queue is accepted as a documented limitation. Those defects undermine the reason to use oly and block 1.0.0.
