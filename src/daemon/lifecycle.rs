...
fn detached_start_summary(config: &AppConfig, no_http: bool, no_auth: bool) -> String {
    ...
    match crate::http::NodeIdentity::read_published_pubkey(&config.paths.state_dir) {
        Ok(Some(pub_key)) => {
            let _ = writeln!(out, "SSH-PUB-KEY:  {}", pub_key);
        }
        Ok(None) => {
            let _ = writeln!(
                out,
                "SSH-PUB-KEY:  (not generated; start the daemon at least once)"
            );
        }
        Err(err) => {
            let _ = writeln!(out, "SSH-PUB-KEY:  failed to read SSH pub key: {}", err);
        }
    }
